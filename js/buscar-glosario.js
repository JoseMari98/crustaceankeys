...
function terminos_a(){
	$.ajax({
      ...
      success: function(respuesta){
        ...
        for (var i = 0; i < respuesta.length; i++){
        		cadena += "<li id='"+respuesta[i]["name"]+"' style='margin-top:5px;'><div><span class='icon' style='margin-left:20px;'></span><a target='_blank' href='termino.html?termino="+respuesta[i]["name"]+"'>"+ respuesta[i]["name"] +"</a>";
        };
        cadena += "</ul>";
		letra += "<a href='#a'>A<a/>";
