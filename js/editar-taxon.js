		      	...
		      	if(respuestafather[0]!= null){
		        	for (var i = 0; i < respuestafather.length; i++){
		          		if(respuestafather[i][0]!=fatherobtenido){
		            		cadena += "<option>"+ respuestafather[i][1].toString() +" "+ respuestafather[i][0].toString() +"</option>";
		            	}else{
		    				cadena += "<option selected='selected'>"+ respuestafather[i][1].toString() +" "+ respuestafather[i][0].toString() +"</option>";
		    			}
		        	};
		    	};
...
function editartaxon(){
	...
	if(taxoncreado != "'Seleccione un taxón'"){
		if(espaciosinifin($("#name").val())!= ""){
		    var taxon = "'"+taxoncreado.substring(taxoncreado.indexOf(" ")+1,taxoncreado.length);
		     taxoncreado = taxon;
		    ...
		    var data = {tipo: 'editartaxon', taxoncreado: taxoncreado, tipotaxon: tipotaxon, name: name, description: description, fatherclassbox: fatherclassbox, synonyms:synonyms}
		    $.ajax({
		      url: '../phpadmin/c-taxones.php',
		      type: 'POST',
		      dataType: 'json',
