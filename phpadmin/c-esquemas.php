	...
	if (isset($_POST['tipo'])){
		...
		switch ($op) {
				...
				if ($_FILES["file"]["name"][0] != ""){
				    ...
				    for ($i=0; $i<count($file['name']); $i++){
				        ...
				        if ($tipo != "image/jpeg" && $tipo != "image/png" && $tipo != "image/jpg" && $tipo != "image/gif"){
				            ...
				            $sql="INSERT INTO `esquemasgenerales` (anchura,altura,tipo,imagen,nombre,descripcion) VALUES ($width,$heigth,'$tipo','$foto',$nombre,'$descripcion')";
				            // echo $sql;
				            $conexion->query($sql);
				            echo "<p style='color: blue'>El esquema $nombre se ha insertado.</p>";
				        };
				    };
			...
			case 'mostrarimagenesquema':
				$esquemacreado = $_POST['esquemacreado'];
				$sql = "SELECT id, anchura, altura, tipo, imagen, nombre, descripcion FROM esquemasgenerales WHERE nombre = $esquemacreado";
				$respuesta = $conexion->query($sql);
			...
			case 'editaresquema':
				...
				$sql = "UPDATE esquemasgenerales SET nombre='$nombre', descripcion='$descripcion' WHERE id='$id'";
				$conexion->query($sql);

				echo json_encode("El Esquema General $nombre ha sido editado.");
			break;
			case 'borraresquema':
				$id = $_POST['id'];
				$nombre = $_POST['nombre'];
				$sql = "DELETE FROM esquemasgenerales WHERE nombre = '$nombre'";
				$conexion->query($sql);
				echo json_encode("La Esquema General $nombre ha sido eliminado.");
			break;
