	...
	if (isset($_POST['tipo'])){
		...
		switch ($op) {
			case 'paginascreadas':
				...
				$sql = "SELECT name FROM paginas order by name asc";
			...
			break;
			case 'rellenarinformacionpagina':
				$paginaseleccionada = $_POST['paginaseleccionada'];
				//Busca y devuelve los datos encontrados en forma de Array
				//Devuelve todos los datos del taxon seleccionado
				$sql = "SELECT contenido FROM paginas WHERE name = '$paginaseleccionada'";
				$respuesta = $conexion->query($sql);
				$respuestas = array();
				array_push($respuestas, $respuesta->fetch_array());
				echo json_encode($respuestas);
			...
			case 'editarpagina':
				...
				$paginacreada = $_POST['paginacreada'];
				$contenido = $_POST['contenido'];
				$sql = "UPDATE paginas SET contenido=$contenido WHERE name = $paginacreada";
				$conexion->query($sql);
				echo json_encode("La página $paginacreada se ha editado correctamente.");
			break;
		}
	}else{
